...
import org.yanislavcore.crawler.data.{FetchSuccess, ScheduledUrlData}
import org.yanislavcore.crawler.{CrawlerConfig, Utils}

import scala.util.Try

class UrlsCollector(cfg: CrawlerConfig) extends FlatMapFunction[(ScheduledUrlData, FetchSuccess), ScheduledUrlData] {
  private val ignoredExtensions = cfg.ignoredExtensions.toSet

  override def flatMap(value: (ScheduledUrlData, FetchSuccess), out: Collector[ScheduledUrlData]): Unit = {
  ...
  private def parseElements(value: (ScheduledUrlData, FetchSuccess), baseUrl: String): Elements = {
